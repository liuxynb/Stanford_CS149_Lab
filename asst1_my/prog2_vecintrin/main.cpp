...
void clampedExpVector(float *values, int *exponents, float *output, int N)
{
  // Declare vector variables for values, exponents, and results
  __cs149_vec_float x;
  __cs149_vec_int y;
  __cs149_vec_float result;

  // Initialize vectors for constants: zero, one, and a clamping threshold
  __cs149_vec_int zero = _cs149_vset_int(0);
  __cs149_vec_int one = _cs149_vset_int(1);
  __cs149_vec_float nine_infinity = _cs149_vset_float(9.999999f);

  // Declare masks for vector operations
  __cs149_mask maskAll, maskUpper, maskNeedMul;
  maskAll = _cs149_init_ones();

  // Loop through the array with vectorized operations
  int i;
  for (i = 0; i <= N - VECTOR_WIDTH; i += VECTOR_WIDTH)
  {
    ...
    // Initialize result vector to 1.0
    _cs149_vset_float(result, 1.f, maskAll);

    // Check if exponent is greater than zero (y > 0)
    _cs149_vgt_int(maskNeedMul, y, zero, maskAll);

    // Perform exponentiation using a loop
    while (_cs149_cntbits(maskNeedMul))
    {
      // Multiply result by x (result *= x)
      _cs149_vmult_float(result, result, x, maskNeedMul);

      // Subtract 1 from y
      _cs149_vsub_int(y, y, one, maskNeedMul);

      // Update mask for the next iteration
      _cs149_vgt_int(maskNeedMul, y, zero, maskNeedMul);
    }

    // Check if result is greater than the clamping threshold (9.999999f)
    _cs149_vgt_float(maskUpper, result, nine_infinity, maskAll);

    // If result is greater, set result to the clamping threshold
    _cs149_vset_float(result, 9.999999f, maskUpper);

    // Store the result vector into the output array
    _cs149_vstore_float(output + i, result, maskAll);
  }

  // Note: This loop indexing assumes (N % VECTOR_WIDTH) == 0. If not, handle remaining elements with a serial version
  if (i != N)
  {
    // Adjust the loop index for the remaining elements
    i -= VECTOR_WIDTH;

    // Handle the remaining elements with a serial version (clampedExpSerial)
...
float arraySumVector(float *values, int N)
{
  ...
  /*
  Notice that VECTOR_WIDTH is a power of 2.

  We have two funcs:
  // Adds up adjacent pairs of elements, so
  //  [0 1 2 3] -> [0+1 0+1 2+3 2+3]
  1. void _cs149_hadd_float(__cs149_vec_float &vecResult, __cs149_vec_float &vec);
  ...
  2. void _cs149_interleave_float(__cs149_vec_float &vecResult, __cs149_vec_float &vec);

  through 1: [1,2,3,4] -> [3,3,7,7]
  through 2: [3,3,7,7] -> [3,7,3,7]
  through 1: [3,7,3,7] -> [10,10,10,10]
  through 2: [10,10,10,10] -> [10,10,10,10]
  10 is the result that we need.
  */
