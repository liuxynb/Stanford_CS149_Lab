...
void sqrt_AVX2(const unsigned int N, const float initialGuess, float *values, float *output)
{
    for (unsigned int i = 0; i < N; i += 8)
    {
        __m256 input = _mm256_loadu_ps(values + i);
        __m256 result = _mm256_sqrt_ps(input);
...
int main()
{
    ...
    for (unsigned int i = 0; i < N; i++)
    {
        ...
        // values[i] = 2.999f; // question 2 answer
        // question 3 answer:
        // if (i % 8 == 0)
        // {
        //     values[i] = 2.999f;
    ...
    // Run the AVX2 manually implementation
    double minAVX2 = 1e30;
    for (int i = 0; i < 3; ++i)
    {
        double startTime = CycleTimer::currentSeconds();
