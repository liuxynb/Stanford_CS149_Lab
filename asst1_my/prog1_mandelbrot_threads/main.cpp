...
int main(int argc, char **argv)
{
    ...
    for (int j = 2; j <= numThreads; j++)
    {
        for (int i = 0; i < 5; ++i)
        {
            ...
            double startTime = CycleTimer::currentSeconds();
            mandelbrotThread(j, x0, y0, x1, y1, width, height, maxIterations, output_thread);
            double endTime = CycleTimer::currentSeconds();
            // to prevent the minThread from being negative, which is impossible
            // but may happen because of the precision of CycleTimer!
